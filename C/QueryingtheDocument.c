...
char**** get_document(char* text) {
    ...
    for(int i = 0; i < MAX_PARAGRAPHS; i++)
        document[i] = (char***)malloc(1 * sizeof(char**));
    for(int i = 0; i < MAX_PARAGRAPHS; i++){
        document[i][0] = (char**)malloc(1 * sizeof(char*));
        document[i][0][0] = (char*)malloc(1 * sizeof(char));
    }
    ...
    for(int n = 0, i = 0, j = 0, k = 0, l = 0; n < strlen(text); n++){
        if(text[n] == ' '){
            ...
            document[i][j][k] = (char*)malloc(sizeof(char));
        }
        else if(text[n] == '.'){
            document[i][j][k][l] = '\0';
            k = l = 0;
            ...
            document[i][j] = (char**)malloc(sizeof(char*));
            document[i][j][k] = (char*)malloc(sizeof(char));
        }
        else if(text[n] == '\n'){
            j = k = l = 0;
